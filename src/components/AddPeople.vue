<template>
    <div class="add-people">
      <button class="add-button" @click="addNewPerson">
        ✙ Добавить человека
      </button>
      ...
      <div class="people-list">
        ...
        <ul>
          <li v-for="(person, index) in peopleStore.people" :key="person.id">
            ...
            />
            <button class="delete-button" @click="removePerson(person.id)">⛌</button>
          </li>
        </ul>
      </div>
  
      <button class="next-button" @click="submitPeople">{{ buttonText }}</button>
    </div>
  </template>
